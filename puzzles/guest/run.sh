java -jar ../../jvm/target/um.jar ../../codex.um <<'FOO' | tee run.out
guest
mail
mail
1
...
D        DIM p AS STRING
DI       DIM d1 AS INTEGER
DII      DIM d2 AS INTEGER
DV       IF i > pwdcount THEN GOTO DLX
DX       d1 = XLVIII
DXI      d2 = XLVIII
DXV      p = words(i) + CHR(d1) + CHR(d2)
DXX      IF CHECKPASS(username, p) THEN GOTO DXLV
DXXV     d2 = d2 + I
DXXX     IF d2 < LVIII THEN GOTO DXV
DXXXI    d1 = d1 + I
DXXXII   IF d1 < LVIII THEN GOTO DXI
DXXXV    i = i + I
DXL      GOTO DV
DXLV     PRINT "found match!! for user " + username + CHR(X)
DL       PRINT "password: " + p + CHR(X)
DLV      END
DLX      PRINT "no match with trailing number for user " + username + CHR(X)
DLXV     END
STOP
/bin/qbasic hack.bas
./hack.exe howie
